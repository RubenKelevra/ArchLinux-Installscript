...
hostname="$1"
[ -z "$2" ] && (echo "supply a maindevice!") && exit 1
maindevice="$2"
[ -z "$3" ] && (echo "supply a swapdevice!") && exit 1
swapdevice="$3"
...
parted -s -- $swapdevice mklabel msdos
parted -a optimal -s -- $swapdevice mkpart primary linux-swap 4096S 99%FREE

mainpartition=$(echo "$maindevice")1
swappartition=$(echo "$swapdevice")1
...
echo "admins=(\"ruben\" \"ernst\")" > /mnt/install.sh
